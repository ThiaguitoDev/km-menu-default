...
Citizen.CreateThread(function()
	...
	openMenu = function(namespace, name, data)
		SendNUIMessage({
			action    = 'openMenu',
			namespace = namespace,
			name      = name,
		...
		if data.tattoo then
			for k, v in pairs(data.elements) do
				table.insert(OpenedValues, v.value)
			end
		end
	end
	while IsTatto == nil do
		Wait(1)
		print("s")
	end
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsControlPressed(0, Keys['TOP']) and GetLastInputMethod(2) and (GetGameTimer() - GUI.Time) > 200 then
				if IsTatto then
					if OpenedType ~= 1 then
						OpenedType = OpenedType - 1
						TriggerEvent("tattoo_test", OpenedValues[OpenedType]['uno'], OpenedValues[OpenedType]['dos'])
					end
				end
				SendNUIMessage({
					action  = 'controlPressed',
			...
			if IsControlPressed(0, Keys['DOWN']) and GetLastInputMethod(2) and (GetGameTimer() - GUI.Time) > 200 then
				if IsTatto then
					if OpenedType ~= #OpenedValues then
						OpenedType = OpenedType + 1
						print(OpenedType)
						print(json.encode(OpenedValues[OpenedType]))
						TriggerEvent("tattoo_test", OpenedValues[OpenedType]['uno'], OpenedValues[OpenedType]['dos'])
					end
				end
				SendNUIMessage({
